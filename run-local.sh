...
# Script to run the BreachWatch application locally (Frontend, Backend, DB)
...
setup_backend_env() {
  ...
  if [ ! -f .env ]; then
    ...
    if [ -f .env.example ]; then
      ...
      echo "Review breachwatch_backend/.env if you have specific configuration needs."
    else
      echo "Warning: breachwatch_backend/.env.example not found. Please create a .env file manually."
      echo "The backend might not start correctly without it."
    fi
  else
...
start_backend_db() {
  echo_status "Starting Backend and Database with Docker Compose..."
  ...
  echo "Backend and Database services started (or starting) in detached mode."
...
start_frontend() {
  ...
  if ps -p $! > /dev/null; then
     echo "Frontend development server started (PID: $!). Logs: frontend_dev.log"
  else
...
cleanup() {
  ...
  # This is a simplistic way; a more robust way would be to save PID.
  # For now, we instruct the user.
  
  echo "Stopping backend and database (docker-compose down)..."
  cd "$(dirname "$0")" # Ensure we are in project root for docker-compose
  ${DOCKER_COMPOSE_CMD} down
  echo "Backend and Database services stopped."
  
  echo "To manually stop the frontend if it's still running:"
  echo "1. Find its PID: ps aux | grep 'next dev.*-p ${FRONTEND_PORT}'"
  echo "2. Kill it: kill <PID>"
  echo "Or, if you started 'npm run dev' in a separate terminal, Ctrl+C there."
...
echo "(Frontend process might need manual stopping if this script's trap doesn't catch it.)"
echo ""
